...
function setup() {
	...
	Again=createButton("Play Again",580,580,20,20);
	Destroy=createButton("Destroy the Hurdle",580,580,20,20);
	Again.hide();
	decrease.hide();
	increase.hide();
...
train.x=1300;
  ...
  increase.mousePressed(()=>{
	train.velocityX=train.velocityX-2;
		});
		decrease.mousePressed(()=>{
	train.velocityX=train.velocityX+2;
		});
if(gameState=="next"){
	...
	go.setVolume(0.002);
	if(frameCount%100==0){
hurdle=createSprite(random(1100,-2000),480,20,20);
	hurdle.addImage(hurdleImage);
	hurdle.scale=0.2;
	hurdle.lifetime=647;
	hurdleGroup.add(hurdle);
	}
	//destroying the hurdle
Destroy.mousePressed(()=>{
	hurdleGroup.destroyEach();
	train.velocityX=-7;
	level2_score=level2_score+10;
	});
	...
	textSize(22);
	fill("black");
	text("For destroying hurdle, click on Destroy the hurdle",train.x-300,110);
	textSize(16);
...
if(gameState=="again"){
	...
	if(train.isTouching(hurdleGroup)){
		...
		textSize(22);
		fill("black");
		text("You have lost.. Click on Play again to start",train.x-300,200);
		Again.show();
		Again.mousePressed(()=>{
gameState="next";
train.x=1300;
  ...
  if(gameState=="next"){
	text("Your Score: "+level2_score,train.x-600,20); 
  }
}
